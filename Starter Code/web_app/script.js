...
async function lookup(debtor, creditor) {
	...
	let debtor_ENS = users[debtor];
	let creditor_ENS = users[creditor];
	return BlockchainSplitwise.lookup(debtor_ENS, creditor_ENS);
}

async function lookupENS(debtor, creditor) {
	return BlockchainSplitwise.lookup(debtor, creditor);
}
...
async function getTotalOwed(user) {
	...
	for (var i = 0; i < users.length; i++) {
	  owedAmount += await lookup(user, users[i]);
	}
	return owedAmount;
}
...
async function getLastActive(user) {
	...
	let userENS = users[user].toLowerCase()
	let functionCalls = await getAllFunctionCalls(contractAddress, 'add_IOU');
	functionCalls = functionCalls
		.filter((functionCall) => functionCall.from === userENS || functionCall.args[0] === userENS)
